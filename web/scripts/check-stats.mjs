...
const segments = statsSegments(full)
check('the line has four segments for a used conversation', segments.length === 4, String(segments.length))
check('turns come first', segments[0].text === '轮 7', segments[0].text)
check(
  'the model segment carries calls and duration',
  segments[1].text === '模型 23 次 · 3m04s',
  segments[1].text,
)
check(
  'the tool segment carries calls and duration',
  segments[2].text === '工具 41 次 · 5.2s',
  segments[2].text,
)
// formatCompact keeps three significant digits below 100 and rounds above it,
// so 853575 renders as 854K rather than 853.6K.
check('the token segment is the compact total', segments[3].text === '854K tokens', segments[3].text)
check('every segment explains itself', segments.every((seg) => typeof seg.title === 'string' && seg.title.length > 8))

// A conversation that has not run anything yet shows the one number that means
// something at zero, and no rows of zeros.
const empty = statsSegments({})
check('an empty conversation shows only the turn count', empty.length === 1 && empty[0].text === '轮 0')
...
const noUsage = statsSegments({ turns: 2, llm_calls: 3, tool_calls: 0 })
check(
  'no reported usage means no token segment',
  noUsage.length === 2 && !noUsage.some((seg) => seg.key === 'tokens'),
)
check('no tool calls means no tool segment', !noUsage.some((seg) => seg.key === 'tools'))

// --- the tooltip --------------------------------------------------------

const title = statsTitle(full)
