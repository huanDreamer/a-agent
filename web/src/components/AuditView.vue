<script setup>
// 审计日志 panel — tool invocations from /api/audit, with a tool-name filter.
// The API also accepts `user`, so an open_id filter is offered next to it.
// A content panel of 统计监控: bare .stack, no page head of its own (the page's
// toolbar gives it 刷新; the range selector would not mean anything here since
// /api/audit takes no since/until).
    ...
    <div class="card">
      <div class="card-head">
        ...
        <div class="row">
          ...
          <span v-if="filterActive" class="chip">命中 {{ formatCount(rows.length) }} 条</span>
