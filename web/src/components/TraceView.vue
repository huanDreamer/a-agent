...
//
// No time-range toolbar here on purpose: `/api/traces` takes its own filters
// (session / name / user), see internal/server/traces.go. It is a content panel
// of 统计监控 — MonitorView owns the page head and the sub-tab strip — so its own
// 重新加载 + host chip live in the first row of the panel.
//
// Three independent panels, each with its own loading / empty / error state.
...
<template>
  <div class="stack">
    <div class="list-toolbar panel-tools">
      <span v-if="host" class="chip mono shrink" :title="host">{{ host }}</span>
      <span class="spacer" />
      <span class="muted-note">/api/traces 使用自己的过滤条件，不受时间范围影响</span>
      <button type="button" class="btn ghost sm" :disabled="list.loading.value" @click="refreshAll">
    ...
    <div class="card">
      <div class="card-head">
        ...
        <span
          v-if="statusRes.status.value === 'ready'"
          class="chip"
          :class="traceEnabled ? 'ok' : 'warn'"
        >
        ...
        <div v-else class="stack">
          ...
          <div v-if="hasDeliveryProblem" class="banner error" role="alert">
            ...
            <span class="banner-text">
              有事件上报失败或被丢弃：请检查 langfuse.host 是否可达、public_key / secret_key
              是否有效。
            </span>
          </div>
        </div>
    ...
    <div class="card">
      ...
      <div class="list-toolbar">
        <button
          type="button"
          class="btn ghost sm"
          :disabled="!canPrev || list.loading.value"
          @click="goPrev"
        >
        ...
        <button
          type="button"
          class="btn ghost sm"
          :disabled="!canNext || list.loading.value"
          @click="goNext"
        >
          下一页
          ...
          <table class="data">
            ...
            <tbody>
              ...
              >
                ...
                <td class="mono dim" :title="trace.session_id || ''">
                  {{ shortId(trace.session_id, 8, 4) }}
                </td>
    ...
    <div class="card">
      <div class="card-head">
        ...
        <div class="row">
          ...
          <button v-if="selectedId" type="button" class="btn ghost sm" @click="selectedId = ''">
