...
1. `context.model_windows`：按模型 id 子串覆盖，最长匹配优先。运维自己的答案，压倒一切。
2. **模型目录里记的窗口**（设置 → 模型管理 → 那一列的「窗口大小」，见下节）：provider 在
   `/models` 里自己报的数（来源标「接口」），或者模型被直接问出来时它自己给的数（标「模型自报」）。
   这一层比内置表可信：网关可能路由到比厂商宣传更小的窗口。
3. 内置的常见模型表（`internal/context/window.go`，四十来条家族条目）。
4. `context.default_window`（默认 128k）。

一个控制台同时服务多个模型，固定数字对它们必然有对有错：60000 会把 200k 窗口的四分之三白白浪费，
又会把 32k 窗口撑爆。启动日志里会打出 `context_window / in_loop_cap / source / derivation`，
设置 → 对话预算也显示同一个数。

### 窗口大小是问出来的，不是猜的

模型管理（设置 → 模型管理）每个模型多了一列**窗口大小**，来源标着它是怎么知道的：

- **接口**：刷新模型时，provider 的 `/models` 里带了 `context_length`（OpenAI 之外的常见拼法
  `context_window` / `max_context_length` / `max_input_tokens` / OpenRouter 的
  `top_provider.context_length` 也都认）。这是 provider 真正会执行的那个数，免费、精确。
- **模型自报**：provider 不报（deepseek 的 `/models` 只给 id），就**直接问模型自己**
  "你的上下文窗口是多少 token"。回答会被解析、按 plausible 区间过滤（拒绝年份、日期、参数数量
  之类的数字），再记下来。这只是一条线索，所以来源标注就摆在旁边。
- 两处触发：**刷新模型**（单个 provider 或「刷新全部」）之后，以及**启动时**补一次。
  都是有限的：一轮最多问 12 个模型、同时最多 2 个、每个 20 秒超时；压缩关掉时整个步骤跳过。

### 估算不准会把窗口撑爆，所以边跑边校准

窗口预算是按"字符数估 token"算的，而估算永远不准：拿真实一轮对照，provider 报的 token 数是估算的
**1.7 倍**（分成中文与 ASCII 计、并把工具 schema 算进去之后仍有这个差距）。128k 窗口上这只是"压缩
比预期晚一点"；到 1M 窗口，`1.7 × 0.7` 就超过 1 —— 历史会越过模型真实上限，然后撞上下文错误。

所以每一步的模型调用都会回报它实际数到的 prompt token，runner 用这个比值校准下一步的预算
（`internal/chat/calibration.go`）：比值只上调不下调（估多了只是早压一次，估少了会毁掉一轮），
上限 3 倍（再高就不是估算误差而是 bug），并且做平滑（中文散文与 Go 源码的比值本来就不一样）。
效果是：除第一步之外，窗口上限是按 provider 自己的计数执行的。
