...
- [x] 1.1 `go get github.com/mark3labs/mcp-go@latest`
- [x] 1.2 `go get gopkg.in/yaml.v3@latest`
- [x] 1.3 `go mod tidy`

## 2. 工具层 (`internal/tool/`)

- [x] 2.1 `internal/tool/tool.go`：
...
- [x] 2.2 `internal/tool/registry.go`：
  ...
  - `(r) Names() []string`
- [x] 2.3 `internal/tool/builtin/time.go`：返回当前时间，支持 timezone 参数
- [x] 2.4 `internal/tool/builtin/calc.go`：用 `github.com/Knetic/govaluate` 或手写解析器求值简单数学表达式
- [x] 2.5 `internal/tool/builtin/echo.go`：原样回显（用于测试）
- [x] 2.6 单元测试：注册/重复/白名单/执行路径

## 3. Agent 循环 (`internal/agent/`)

- [x] 3.1 `internal/agent/agent.go`：
...
- [x] 5.2 `internal/skill/loader.go`：
  - `type Loader struct { dir string }`
  - `func NewLoader(dir string) *Loader`
  - `(l) LoadAll() ([]*Skill, error)` —— 扫目录下所有 `.md`，错误累积返回
  - `(l) Get(name string) (*Skill, bool)`
- [x] 5.3 `configs/skills/daily-summary.md`：内置模板
- [x] 5.4 `configs/skills/code-review.md`：内置模板
- [x] 5.5 单元测试：frontmatter 解析、错误累积、查不到 name

## 6. 配置扩展

- [x] 6.1 `internal/config/config.go`：
...
- [x] 6.2 `configs/config.example.yaml` 添加示例

## 7. Chat CLI 扩展

- [x] 7.1 `cmd/huan-agent/chat.go` 新增 flags：
  - `--tools`：逗号分隔的工具名白名单
  - `--skill`：单个 skill 名称
...
- [x] 8.1 `internal/store/migrate.go`：新增 v2 migration 建 `tool_invocations` 表
- [x] 8.2 `internal/store/audit.go`：`RecordInvocation` / `QueryInvocations`
- [x] 8.3 单元测试

## 9. 验证

- [x] 9.1 `make build` 成功
- [x] 9.2 `make test` 全绿，所有 internal 包覆盖率 ≥ 70%
- [x] 9.3 端到端 smoke（不调真实 provider）：
  - 写一个 mock LLM 跑通"用户问时间 → agent 调 time 工具 → 返回给用户"
  - 验证 `tool_invocations` 表里有记录
- [x] 9.4 mcp-go in-memory server 跑通 ListTools / CallTool
- [x] 9.5 skill loader 能加载 `daily-summary` / `code-review` 两个模板

## 10. 收尾

- [x] 10.1 起草 Phase 3 change proposal
- [x] 10.2 归档 Phase 2
